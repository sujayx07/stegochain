    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {Array.from({ length: count }).map((_, i) => (
          <div key={i} style={{ borderRadius: 14, border: "1.5px solid #EBEBEB", overflow: "hidden" }}>
            <div style={{ height: 3, background: "linear-gradient(90deg,#F5F5F5,#E8E4DF,#F5F5F5)", backgroundSize: "200% 100%", animation: "shimmer 1.6s ease-in-out infinite" }} />
  ...
  if (type === "stat") {
    return (
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
        {Array.from({ length: count }).map((_, i) => (
          <div key={i} style={{ flex: 1, minWidth: 160, padding: "24px 28px", background: "white", borderRadius: 14, border: "1.5px solid #EBEBEB" }}>
  ...
  if (type === "list") {
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {Array.from({ length: count }).map((_, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", background: "white", borderRadius: 10, border: "1px solid #EBEBEB" }}>
  ...
  if (type === "table") {
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
        {Array.from({ length: count }).map((_, i) => (
          <div key={i} style={{ display: "flex", gap: 16, padding: "14px 16px", background: i % 2 === 0 ? "white" : "#FFFFFF", borderBottom: "1px solid #F5F5F5" }}>
